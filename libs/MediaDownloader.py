...
def _sanitize_source(source: str) -> str:
    """
    Remove markdown link formatting [label](url) → url.
    Handles cases where the source string accidentally contains markdown syntax.
    """
    if not source:
        return source
    match = re.match(r'^\[.*?\]\((https?://[^)]+)\)$', source.strip())
    if match:
...
class MediaDownloader:
    ...
    def resolve_source_path(source: str, temp_dir: str = "output/temp",
        ...
        # Sanitiza markdown links: [label](url) → url
